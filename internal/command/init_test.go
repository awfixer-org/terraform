...
func TestInit_stateStore_to_backend(t *testing.T) {
	...
	}
	{
		...
		testBackend := new(testHttpBackend)
...
`, ts.URL)
		...
		data, err := testBackend.StatefileData()
		if err != nil {
			t.Fatal(err)
		}
...
func (h *testHttpBackend) StatefileData() (*statefile.File, error) {
	return statefile.Read(bytes.NewBuffer(h.Data))
}
